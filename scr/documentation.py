...
def documentations():
    ...
    st.write("""To use the Hugging Face API, you need to obtain an access
             key. Follow these steps to get your key:""")
    st.markdown("""1. Go to the [Hugging Face website]
    ...
    st.markdown("""<a href=https://huggingface.co/docs/hub/security-tokens>
                For informations </a>""", unsafe_allow_html=True)
    st.header("Setting Hugging Face Access Key in Streamlit")
    st.write("""To use the Hugging Face API in your Streamlit app, you need to
             set the access key as an environment variable. Follow these
             steps:""")
    st.markdown("1. Open your application")
    st.markdown("""2. At the bottom of your Streamlit app, click on manage app
                button and then select the 3 horizontal dots and then go to
                the settings and select now secrets to remplace the new
                acces key""")
    ...
    st.write("""Now you can use the Hugging Face API in your Streamlit app
             without any issues.""")
